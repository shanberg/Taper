...
describe('ScheduleRow', () => {
  ...
  beforeEach(() => {
    ...
    (formatRowText as jest.Mock).mockImplementation(({ row, rowStartDate, rowEndDate, index, selectedLanguageKey }) => {
  ...
  test('renders correctly', () => {
    const { container } = render(ScheduleRow, { tableData, startDate, selectedLanguageKey, row, index });
  ...
  test('formats the row text correctly', () => {
    const { getByText } = render(ScheduleRow, { tableData, startDate, selectedLanguageKey, row, index });
