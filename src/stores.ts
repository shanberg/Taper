...
export function createAppStore(): AppStore {
    ...
    return {
        ...
        editSegmentAtIndex: (index: number, updatedSegment: Segment): void => update(state => {
        ...
        changeStartDate: (newDate: ScheduleDate | InputStringDate): void => update(state => {
            const newState = _saveScheduleForUndo(state);
            const newTaperDate = newDate === '' ? new TaperDate().incrementByOneDay() : new TaperDate(newDate);
            return {
                ...newState,
        ...
        insertPlaceholderSegmentBeforeIndex: (index: number): void => update(state => {
            const newState = _saveScheduleForUndo(state);
        ...
        switchTemplate: (newTemplateKey: string): void => update(state => {
            const newState = _saveScheduleForUndo(state);
            return {
                ...newState,
                schedule: { ...newState.schedule, segments: [...TEMPLATES[newTemplateKey], { ...PLACEHOLDER_SEGMENT }], templateKey: newTemplateKey }
            };
        }),
        deleteSegmentAtIndex: (index: number): void => update(state => {
            const newState = _saveScheduleForUndo(state);
            const newSegments = [...newState.schedule.segments];
            newSegments.splice(index, 1);
            return { ...newState, schedule: { ...newState.schedule, segments: newSegments } };
        }),
        undo: (): void => update(state => {
